...
#define COMMON_SETTINGS(M) \
    ...
    M(Bool, experimental_query_result_cache_active_usage, false, "Store results of SELECT queries in query result cache", 0) \
    M(Bool, experimental_query_result_cache_passive_usage, false, "Try to answer SELECT queries from query result cache", 0) \
    M(Bool, share_query_result_cache_between_users, false, "Share entries in query result cache between users", 0) \
    M(UInt64, min_query_runs_for_query_result_cache, 3, "Minimum number a SELECT query must run before its result is stored in the query result cache", 0) \
    M(UInt64, query_result_cache_max_entry_size, 250 * (1ULL << 20), "Maximum size in bytes of SELECT query results to be saved in query result cache", 0) \
